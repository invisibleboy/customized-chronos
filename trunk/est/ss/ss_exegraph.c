...
add_inst(int inst)
{
	...
	// qali-rev: add memory latency into pipeline stage
	...
	else if (stage == STAGE_WB )
	{
		...
		{
			node->lat.lo = node->lat.hi = MEM_LOAD;
		}
		else if((eg_insts_type[inst] == INST_STORE))
		{
			node->lat.lo = node->lat.hi = MEM_STORE;
		}
	}		
	else {
	    node->fu = node->num_fu = 0;
